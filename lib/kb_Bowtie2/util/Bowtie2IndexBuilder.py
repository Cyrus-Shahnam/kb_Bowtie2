...
class Bowtie2IndexBuilder(object):
    ...
    def _validate_params(self, params):
        ...
        if os.path.exists(validated_params['output_dir']):
            raise ValueError('Output directory name specified (' + validated_params['output_dir'] +
        ...
        # NEW: SAIS indexing preference (bool-ish)
        if 'use_sais' in params:
            validated_params['use_sais'] = self._truthy(params['use_sais'])
        else:
            validated_params['use_sais'] = False
    ...
    def _build_index(self, assembly_info, validated_params):
        ...
        print('bowtie2-build params:', cli_params)
    ...
    def _build_cli_params(self, fasta_file_path, index_files_basename, validated_params):
        ...
        # NEW: SAIS indexing (Bowtie2 ≥ 2.5.3)
        if validated_params.get('use_sais'):
            cli_params.append('--sais')
